...
U8G2_SSD1322_NHD_256X64_F_4W_HW_SPI u8g2(U8G2_R2, /* cs=*/ 42, /* dc=*/ 40, /* reset=*/ 41); // Hardware SPI 3.12inch OLED
//U8G2_SH1122_256X64_F_4W_HW_SPI u8g2(U8G2_R0, /* cs=*/ 42, /* dc=*/ 40, /* reset=*/ 41);				// Enable U8G2_16BIT in u8g2.h 2.08inch OLED
...
static unsigned char sdcard[] PROGMEM = {
   0xe0, 0xff, 0xff, 0xff, 0x0f, 0xf0, 0xff, 0xff, 0xff, 0x1f, 0xf0, 0xff,
   0xff, 0xff, 0x3f, 0xf0, 0x7f, 0x7c, 0xfc, 0x3f, 0x70, 0x44, 0x44, 0x44,
   0x3c, 0x70, 0x44, 0x44, 0x44, 0x3c, 0x70, 0x44, 0x44, 0x44, 0x3c, 0x70,
   0x44, 0x44, 0x44, 0x3c, 0x70, 0x44, 0x44, 0x44, 0x3c, 0x70, 0x44, 0x44,
   0x44, 0x3c, 0x70, 0x44, 0x44, 0x44, 0x3c, 0x70, 0x44, 0x44, 0x44, 0x3c,
   0x70, 0x44, 0x44, 0x44, 0x3c, 0xf0, 0xff, 0xff, 0xff, 0x3f, 0xf0, 0xff,
   0xff, 0xff, 0x3f, 0xf0, 0xff, 0xff, 0xff, 0x3f, 0xf0, 0xff, 0xff, 0xff,
   0x3f, 0xf8, 0xff, 0xff, 0xff, 0x3f, 0xfc, 0xff, 0xff, 0xff, 0x3f, 0xfe,
   ...
   0xff, 0xff, 0x3f, 0xfc, 0xff, 0xff, 0xff, 0x3f, 0xfc, 0x03, 0x0c, 0xe0,
   0x3f, 0xfc, 0x01, 0x0c, 0xc0, 0x3f, 0xfc, 0x00, 0x0c, 0x80, 0x3f, 0x7c,
   0xf0, 0x8f, 0x87, 0x3f, 0x7e, 0xf8, 0x8f, 0x0f, 0x3f, 0x7f, 0xf0, 0x8f,
   0x1f, 0x3f, 0xff, 0x00, 0x8f, 0x1f, 0x3f, 0xff, 0x00, 0x8e, 0x1f, 0x3f,
   0xff, 0x01, 0x8c, 0x1f, 0x3f, 0xff, 0x3f, 0x8c, 0x1f, 0x3f, 0xff, 0x7f,
   0x8c, 0x0f, 0x3f, 0xff, 0x3f, 0x8c, 0x87, 0x3f, 0xff, 0x00, 0x0c, 0x80,
   0x3f, 0x7f, 0x00, 0x0e, 0xc0, 0x3f, 0xff, 0x00, 0x0f, 0xe0, 0x3f, 0xff,
   0xff, 0xff, 0xff, 0x3f, 0xff, 0xff, 0xff, 0xff, 0x3f, 0xff, 0xff, 0xff,
   0xff, 0x3f, 0xff, 0xff, 0xff, 0xff, 0x3f, 0xff, 0xff, 0xff, 0xff, 0x3f,
...
void displayRadio()
{
  Serial.println("debug---displayRado inside");

  u8g2.clearBuffer();	
  u8g2.setFont(u8g2_font_fub14_tf);
  stationName = stationName.substring(0, 25);
  u8g2.drawStr(0, 14, stationName.c_str());
  
  // Funkcja wyswietlania numeru Banku i Stacji w górnym prawym rogu - prototyp
  //u8g2.setFont(u8g2_font_mozart_nbp_tf);
  u8g2.setFont(spleen6x12PL);
  char BankStationStr[8]; 
  snprintf(BankStationStr, sizeof(BankStationStr), "S%02d.%02d", bank_nr, station_nr); //Formatowanie informacji o stacji i baku do postaci S-1699 
...
void setup()
{
  ...
  if (!SD.begin(SD_CS_PIN, customSPI))
  {
    ...
    u8g2.setDrawColor(1);
    u8g2.drawXBMP(210, 5, 38, 50, sdcard); // ikona SD karty   
    u8g2.sendBuffer();	
    return;
  }
  ...
  if (wifiManager.autoConnect("ESP Internet Radio"))
  {
    ...
    u8g2.setFont(spleen6x12PL);
    u8g2.clearBuffer();
    u8g2.drawStr(10, 25, "Pobieranie stacji z:");	
  ...
  else
  {
    ...
    u8g2.clearBuffer();	
    u8g2.setFont(spleen6x12PL);
    u8g2.drawStr(5, 23, "No network connection"); // W przypadku braku polaczenia wifi - wyswietl komunikat na wyswietlaczu OLED
    u8g2.drawStr(5, 36, "Connect to WiFi: ESP Internet Radio");
    u8g2.drawStr(5, 49, "Open http://192.168.4.1");
    u8g2.sendBuffer();	
  }
