...
#define softwareRev "v3.13"  // wersja oprogramowania
...
#define rcCmdVolumeUp 0x9D6228D7
#define rcCmdVolumeDown 0x9D62A857
#define rcCmdBankUp 9D62D02F
#define rcCmdBankDown 9D6250AF
#define rcCmdStationUp 0x9D62E11E
#define rcCmdStationDown 0x9D62619E
#define rcCmdKey0 0x00
#define rcCmdKey1 9D62807F
#define rcCmdKey2 9D6240BF
#define rcCmdKey3 0x03
#define rcCmdKey4 0x04
#define rcCmdKey5 0x05
#define rcCmdKey6 0x06
#define rcCmdKey7 0x07
#define rcCmdKey8 0x08
#define rcCmdKey9 0x09
...
int toneLowValue = 0;               // Wartosc filtra dla tonow niskich
int toneMidValue = 0;               //  Wartosc flitra dla tonow srednich
int toneHiValue = 0;                //Wartość filtra tonow wysokich
...
unsigned int *psramData;                  // zmienna do trzymania danych stacji w pamieci PSRAM
    ...
    else
    {
      ...
      if (bit_count == 32)
      {
        ...
        if ((ADDR ^ IADDR) == 0xFF && (CMD ^ ICMD) == 0xFF)
        {
          ...
          if (ir_code == rcCmdVolumeUp)      // Przycisk w prawo
          { 
            Serial.println("debug--IR Vol+");
            volumeUpFlag = true;
          } 
          else if (ir_code == rcCmdVolumeDown) // Przycisk w lewo
          {  
            Serial.println("debug--IR Vol-");
             volumeDownFlag = true;
          }
          else if (ir_code == rcCmdStationUp) // Przycisk w górę 9D62D02F  9D62E11E
          {  
            Serial.println("debug--IR Up Station+");
            station_nr++;
            if (station_nr > stationsCount) 
            {
              station_nr = stationsCount;
            }
            stationChangeFlag = true;
          }
          else if (ir_code == rcCmdStationDown) // Przycisk w dół
          {  
            Serial.println("debug--IR Up Station-");
            station_nr--;
            if (station_nr < 1) 
...
void setup() {
  ...
  if (wifiManager.autoConnect("ESP Internet Radio")) {
    ...
    timer2.attach(0.0003,analyzePulseFromIR);
...
*/
  ...
  if (action4Taken = false)  // aktualizujemy zegar i wyswietlacz ale tylko raz przy starcie zanim wejdziemy do funkcji millis odswiezania scrollera
  {
    displayRadio();
    updateTimer();
    action4Taken = true;
  ...
  if (stationChangeFlag == true)
  {
    stationChangeFlag = false;
    changeStation();
    } 

  if (volumeUpFlag == true)
  {
    volumeUpFlag = false;
    volumeUp();
  }

  if (volumeDownFlag == true)
  {
